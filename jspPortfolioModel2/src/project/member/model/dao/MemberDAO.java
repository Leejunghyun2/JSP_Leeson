...
public class MemberDAO {
	...
	public int getTotalRecord(MemberDTO paramDto) {
		...
		conn = DB.dbConn();
		try {
			String sql = "select count(*) totalRecord from member where 1 = 1";
	...
	public ArrayList<MemberDTO> getSelectAll(MemberDTO paramDto){
		...
		conn = DB.dbConn();
		try {
			String basicSql = "select * from member where 1 = 1";
	...
	public MemberDTO getLogin(MemberDTO paramDto) {
		...
		conn = DB.dbConn();
		try {
			String sql = "select no, name, id from member where id= ? and passwd =?";//"select * from member where no = ?";
	...
	public int setUpdate(MemberDTO paramDto) {
		...
		try {
			...
			sql += " phone1 = ?, phone2 = ?, phone3 = ?, email1 = ?, email2 = ?, postcode = ?, address = ?, detailAddress = ?, extraAddress = ? ";
